...
class Electrodialysis0DData(UnitModelBlockData):
    ...
    **Valid values:** {
    ...
    def build(self):
        ...
        #solution conductivity - To be included into the property package 

        self.slt_eq_conductivity = Var(
            initialize = 0.001,
            units = pyunits.meter ** 2 * pyunits.ohm ** -1 * pyunits.mol ** -1,
            doc = 'solution equivalent conductivity'
        ) # To be included in the property pack
       ...
       # 
        ...
        @self.Constraint(self.flowsheet().config.time,
                         self.config.property_package.phase_list,
                         doc="Current calculation from voltage in the 'Constant Voltage' mode") 

        def eq_current_from_voltage(self, t, p):
            if self.config.operation_mode == 'Constant Voltage':
                return self.current / (self.cell_width * self.cell_length) == self.voltage / (self.membrane_surface_resistence['aem'] + self.membrane_surface_resistence['cem'] \
                    + self.spacer_thickness * (self.slt_eq_conductivity ** -1  * sum(self.concentrate_channel.properties_out[t].conc_mol_phase_comp[p, j] for j in ion_set) ** -1 + \
    ...
    def calculate_scaling_factors(self):
        ...
        if iscale.get_scaling_factor(self.membrane_surface_resistence, warning=True) is None:
            iscale.set_scaling_factor(self.membrane_surface_resistence, 1e4)

        #Scaling facors for  constraints
        if self.eq_current_from_voltage.is_constructed:
